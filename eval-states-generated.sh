    ...
    do
        ...
        then
            ...
            ${BIN} \
                ...
                --use-cardinality-file "${NAME}.cardinalities.json" \
                --statistics \
                --plan-enumerator HeuristicSearch --hs-vertex SubproblemsArray --hs-heuristic zero --hs-search AStar \
                "${NAME}.schema.sql" \
                "${NAME}.query.sql" \
        ...
        ${BIN} \
            ...
            --use-cardinality-file "${NAME}.cardinalities.json" \
            --statistics \
            --plan-enumerator HeuristicSearch --hs-vertex SubproblemsArray --hs-heuristic sum --hs-search AStar \
            "${NAME}.schema.sql" \
            "${NAME}.query.sql" \
            2>&1 | ack 'Vertices generated:' | cut -d ':' -f 2 | tr -d ' '
        set +x
