...
def create_cyclic_two_vertex_cut_join_graph_for_large_join(number_of_relations: int):
    ...
    max_add = int(math.floor(number_of_relations / 2))
    for i in range(1, max_add):
        joins.append(
            Join(relations[number_of_relations - 1 - i], relations[i], ["fk3"], ["fk3"])
        )

    return JoinGraph(relations, joins)

def create_star_schema(number_of_dimension_tables: int, selectvities: list[int]):
    relations: list[Relation] = []
    joins: list[Join] = []
    ...
    relations.extend([Relation(f"dim_0", f"d_0", ["id"], [f"d_0.id < {selectvities[0]}"]), Relation(f"dim_1", f"d_1", ["id"], [f"d_1.id < {selectvities[1]}"]), Relation(f"dim_2", f"d_2", ["id"], [f"d_2.id < {selectvities[2]}"]), Relation(f"dim_3", f"d_3", ["id"], [f"d_3.id < {selectvities[3]}"])])
...
def create_redundancy_cycle_join(selectivity: int):
    relations: list[Relation] = []
    joins: list[Join] = []
...
def create_redundancy_tvc_join(selectivity: int):
    relations: list[Relation] = []
    joins: list[Join] = []
