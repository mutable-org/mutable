...
#undef DEF_COUNTER
    ...
    void push(state_type state, double h) {
        ...
        if (auto it = states_.find(state); it == states_.end()) [[likely]] {
            /*----- Entirely new state, never seen before. -----*/
            it = states_.emplace_hint(it, std::move(state), StateInfo(h, &Q));
    ...
    void push_regular_queue(state_type state, double h) {
        if constexpr (HasRegularQueue) {
            push<false>(std::move(state), h);
        } else {
            ...
            if (auto it = states_.find(state); it == states_.end()) {
                ...
                it = states_.emplace_hint(it, std::move(state), StateInfo(h, &regular_queue_));
                inc_new();
    ...
    void push_beam_queue(state_type state, double h) { push<true>(std::move(state), h); }
    ...
    typename map_type::const_iterator find(const state_type &state) { return states_.find(state); }
    typename map_type::const_iterator end() const { return states_.end(); }
...
#undef X
    ...
    friend std::ostream & operator<<(std::ostream &out, const StateManager &SM) {
        ...
        out << ", " << SM.states_.size() << " seen, " << SM.regular_queue_.size() << " currently in regular queue, "
...
#undef DEF_COUNTER
    ...
    void beam(state_type state, double h) {
        ...
        if (candidates.size() < beam_width) {
            ...
            state_manager_.push_regular_queue(std::move(state), h);
        } else {
...
#endif
            state_manager_.push_regular_queue(std::move(worst_candidate.state), worst_candidate.h); // move to regular
        }
    };
    ...
    void beam_dynamic() {
        ...
        for (auto end = it + num_beamed; it != end; ++it)
            state_manager_.push_beam_queue(std::move(it->state), it->h);
        /*----- Add remaining states to the regular queue. -----*/
        for (auto end = candidates.end(); it != end; ++it)
            state_manager_.push_regular_queue(std::move(it->state), it->h);
    };
                                   ...
                                   expand_type &expand, Context&... context)
    {
        ...
        if (auto it = state_manager_.find(state); it == state_manager_.end()) {
                                    ...
                                    expand_type &expand, Context&... context)
    {
        ...
        expand(state, [this, callback=std::move(callback), &heuristic, &context...](state_type successor) {
            if (auto it = state_manager_.find(successor); it == state_manager_.end()) {
    ...
    void explore_state(const state_type &state, heuristic_type &heuristic, expand_type &expand, Context&... context) {
        if constexpr (use_dynamic_beam_sarch) {
            ...
            beam_dynamic(); // order by g+h and partition into beam and regular states
        } else if constexpr (use_beam_search) {
            /*----- Keep only `beam_width` best successors in `candidates`, rest is added to regular queue. ----*/
            candidates.clear();
            for_each_successor([this](state_type successor, double h) {
                beam(std::move(successor), h); // try to add to candidates
            }, state, heuristic, expand, context...);
            /*----- The states remaining in `candidates` are within the beam. -----*/
            for (auto &s : candidates)
                state_manager_.push_beam_queue(std::move(s.state), s.h);
        } else {
            /*----- Have only regular queue. -----*/
            for_each_successor([this](state_type successor, double h) {
                state_manager_.push_regular_queue(std::move(successor), h);
            }, state, heuristic, expand, context...);
        }
    };
...
double genericAStar<State, Heuristic, Expand, Weight, BeamWidth, Lazy, IsMonotone, Context...>::search(
    ...
    state_manager_.template push<use_beam_search and is_monotone>(std::move(initial_state), 0);
