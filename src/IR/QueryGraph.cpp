...
struct m::GetCorrelationInfo : ConstASTExprVisitor
{
    ...
    struct CorrelationInfo : ASTExprVisitor
    {
        ...
        std::unordered_map<const Designator*, Designator*> old_to_new_outer_; ///< maps old designators to new ones generated by the caller
        std::unordered_map<const Designator*, Designator*> old_to_new_inner_; ///< maps old designators to new ones generated by `replace()`
        std::unordered_set<const Designator*> replaced_designators_; ///< all replaced designators which have to be deleted
        ...
        void replace(std::unordered_map<const Designator*, Designator*> &old_to_new, const char *table_name,
                     std::vector<const Expr*> &expansion) {
        ...
        void operator()(Const<UnaryExpr> &e) override {
            if (finishDecorrelation_) {
                ...
                if (auto d = cast<Designator>(e.expr); d and contains(uncorrelated_exprs, d)) {
                    /* Replace this designator by an equivalent new one. Set table name iff present. */
                    e.expr = table_name_ ? set_table_name(get_new(d)) : get_new(d);
        ...
        void operator()(Const<BinaryExpr> &e) override {
            if (finishDecorrelation_) {
                ...
                if (auto d = cast<Designator>(e.lhs); d and contains(uncorrelated_exprs, d)) {
                    /* Replace this designator by an equivalent new one. Set table name iff present. */
                    e.lhs = table_name_ ? set_table_name(get_new(d)) : get_new(d);
                    replaced_designators_.emplace(d);
                } else {
                    (*this)(*e.lhs);
                }
                if (auto d = cast<Designator>(e.rhs); d and contains(uncorrelated_exprs, d)) {
                    /* Replace this designator by an equivalent new one. Set table name iff present. */
                    e.rhs = table_name_ ? set_table_name(get_new(d)) : get_new(d);
                    replaced_designators_.emplace(d);
                } else {
                    (*this)(*e.rhs);
                }
        ...
        Designator * get_new(Designator *d) {
...
struct m::Decorrelation
{
    ...
    bool decorrelate() {
        ...
        if (graphs_.size() > 1) {
            ...
            for (auto des : needed_attrs_) {
                /* Skip `FnApplicationExpr`s because they have no table name. */
                if (auto d = cast<const Designator>(des); d and not d->has_explicit_table_name())
    ...
    void pushPredicates(QueryGraph *lower, QueryGraph *upper, Query *query) {
        ...
        for (auto it = equi.begin(); it != non_equi.end(); ++it) {
            ...
            std::unordered_map<const Designator*, Designator*> old_to_new;
             ...
             * sources are only reachable via `query`. */
            i->replace(old_to_new, query->alias(), lower->info_->expansion_);
