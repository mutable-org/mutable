...
namespace
{
    ...
    __attribute__((constructor(201))) static void add_wasm_operator_args()
    {
        ...
        C.arg_parser().add<bool>(
            ...
            /* callback=    */ [](bool)
            { options::hash_based_group_join = false; });
        C.arg_parser().add<const char *>(
            /* group=       */ "Wasm",
            /* short=       */ nullptr,
        ...
        C.arg_parser().add<std::size_t>(
            ...
            /* callback=    */ [](std::size_t num_tuples)
            { options::soft_pipeline_breaker_num_tuples = num_tuples; });
        C.arg_parser().add<std::size_t>(
            /* group=       */ "Wasm",
            /* short=       */ nullptr,
            ...
            /* callback=    */ [](std::size_t size)
            { options::index_sequential_scan_batch_size = size; });
        C.arg_parser().add<std::size_t>(
            /* group=       */ "Wasm",
            /* short=       */ nullptr,
            ...
            /* callback=    */ [](std::size_t size)
            { options::result_set_window_size = size; });
        C.arg_parser().add<bool>(
            /* group=       */ "Wasm",
            /* short=       */ nullptr,
            ...
            /* callback=    */ [](bool)
            { options::exploit_unique_build = false; });
        C.arg_parser().add<bool>(
            /* group=       */ "Wasm",
            /* short=       */ nullptr,
            /* long=        */ "--no-simd",
        ...
        C.arg_parser().add<bool>(
            ...
            /* callback=    */ [](bool)
            { options::double_pumping = false; });
        C.arg_parser().add<std::size_t>(
            /* group=       */ "Wasm",
            /* short=       */ nullptr,
            /* long=        */ "--simd-lanes",
            /* description= */ "set the number of SIMD lanes to prefer",
            /* callback=    */ [](std::size_t lanes)
            { options::simd_lanes = lanes; });
        C.arg_parser().add<std::vector<std::string_view>>(
            /* group=       */ "Hacks",
            /* short=       */ nullptr,
                      ...
                      const m::wasm::MatchBase &child)
{
    ...
    { // result set contains only NULL constants
        if (window_size != 0)
        { // i.e. result set is materialized only partially
        ...
        { // i.e. result set is materialized entirely
            /*----- Create child function s.t. result set is extracted in case of returns (e.g. due to `Limit`). -----*/
            FUNCTION(child_pipeline, void(void))
            {
                auto S = CodeGenContext::Get().scoped_environment(); // create scoped environment for this function
    ...
    { // result set contains contains actual values
        if (window_size != 0)
        { // i.e. result set is materialized only partially
            M_insist(window_size > CodeGenContext::Get().num_simd_lanes());
            M_insist(window_size % CodeGenContext::Get().num_simd_lanes() == 0);
                       ...
                       const Schema &schema, std::size_t aggregates_offset = 0)
{
    ...
    for (std::size_t i = aggregates_offset; i < schema.num_entries(); ++i)
    {
        ...
        if (fn.fnid == m::Function::FN_AVG)
        {
            ...
            { // insert additional running count
            ...
            if (fn_expr.args[0]->type()->size() <= 32)
            {
                ...
                auto pred = [&fn_expr](const auto &_fn_expr)
                {
                    ...
                    return _fn_expr.get().get_function().fnid == m::Function::FN_SUM and * _fn_expr.get().args[0] == *fn_expr.args[0];
                };
                if (auto it = std::find_if(aggregates.cbegin(), aggregates.cend(), pred);
                    it != aggregates.cend())
                ...
                { // insert additional SUM aggregate
                    std::ostringstream oss;
                    oss << "$sum_" << fn_expr;
...
decompose_equi_predicate(const cnf::CNF &cnf, const Schema &schema_left)
{
    ...
    for (auto &clause : cnf)
    {
        ...
        M_insist((not literal.negative() and binary.tok == TK_EQUAL) or
                     (literal.negative() and binary.tok == TK_BANG_EQUAL),
...
index_scan_bounds_t extract_index_scan_bounds(const cnf::CNF &cnf)
{
    ...
    for (auto &clause : cnf)
    {
        ...
        M_insist((is<const Designator>(binary.lhs) and is_valid_bound(*binary.rhs)) or
                     (is<const Designator>(binary.rhs) and is_valid_bound(*binary.lhs)),
                 "invalid predicate");
...
void Scan<SIMDfied>::execute(const Match<Scan> &M, setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    ...
    const std::size_t num_simd_lanes =
        SIMDfied ? (options::double_pumping ? 2 : 1) * std::max(num_simd_lanes_preferred, get_num_simd_lanes(table.layout(), layout_schema, schema))
                 : 1;
                                                                    ...
                                                                    const ScanOperator *> &partial_inner_nodes)
{
    ...
    for (auto &clause : cnf)
    {
        ...
        default:
            return ConditionSet::Make_Unsatisfiable();
        case TK_EQUAL:
            if (not has_lo_bound and not has_hi_bound)
            { // lo and hi bound not yet set
        ...
        case TK_GREATER:
        case TK_GREATER_EQUAL:
            if (has_attribute_left and not has_lo_bound)
            { // attribute on lhs, lo bound not yet set
                has_lo_bound = true;
            }
            else if (not has_attribute_left and not has_hi_bound)
            { // attribute on rhs, hi bound not yet set
                has_hi_bound = true;
            }
            else
            {
        ...
        case TK_LESS:
        case TK_LESS_EQUAL:
            if (has_attribute_left and not has_hi_bound)
            { // attribute on lhs, hi bound not yet set
                has_hi_bound = true;
            }
            else if (not has_attribute_left and not has_lo_bound)
            { // attribute on rhs, lo bound not yet set
                has_lo_bound = true;
            }
            else
            {
...
#define RESOLVE_INDEX_METHOD(ATTRTYPE, SQLTYPE) \
    ...
    visit(overloaded{
              [&](const Boolean &)
              { RESOLVE_INDEX_METHOD(bool, _Boolx1); },
              [&](const Numeric &n)
              {
                  switch (n.kind)
                  {
                  ...
                  case Numeric::N_Decimal:
                      ...
                      {
                      default:
                          M_unreachable("invalid size");
                      case 8:
                          RESOLVE_INDEX_METHOD(int8_t, _I8x1);
                          break;
                      case 16:
                          RESOLVE_INDEX_METHOD(int16_t, _I16x1);
                          break;
                      case 32:
                          RESOLVE_INDEX_METHOD(int32_t, _I32x1);
                          break;
                      case 64:
                          RESOLVE_INDEX_METHOD(int64_t, _I64x1);
                          break;
                      }
                      break;
                  case Numeric::N_Float:
                      ...
                      default:
                          M_unreachable("invalid size");
                      case 32:
                          RESOLVE_INDEX_METHOD(float, _Floatx1);
                          break;
                      case 64:
                          RESOLVE_INDEX_METHOD(double, _Doublex1);
                          break;
                      }
                      break;
                  }
              ...
              { RESOLVE_INDEX_METHOD(const char *, NChar); },
              [&](const Date &)
              { RESOLVE_INDEX_METHOD(int32_t, _I32x1); },
              [&](const DateTime &)
              { RESOLVE_INDEX_METHOD(int64_t, _I64x1); },
    ...
    const std::tuple<const ProjectionOperator *> &partial_inner_nodes)
{
    ...
    if (not projection.children().empty())
    { // projections starting a pipeline produce only a single tuple, i.e. no SIMD
...
void Projection::execute(const Match<Projection> &M, setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    auto execute_projection = [&, pipeline = std::move(pipeline)]()
    {
        ...
        for (auto &e : M.projection.schema())
        {
            if (not new_env.has(e.id) and not e.id.is_constant())
            { // no duplicate and no constant
                if (old_env.has(e.id))
                {
                ...
                else
                {
                    ...
                    std::visit(overloaded{
                                   ...
                                   [](std::monostate) -> void
                                   { M_unreachable("invalid expression"); },
                               },
    ...
    if (M.child)
    {
        ...
        {
            visit(overloaded{[](const m::ast::ErrorExpr &) -> void
...
void OrderedGrouping::execute(const Match<OrderedGrouping> &M, setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    ...
    auto store_locals_to_globals = [&]()
    {
            ...
            case m::Function::FN_MAX:
            {
                ...
                case Numeric::N_Decimal:
                    ...
                    default:
                        M_unreachable("invalid size");
                    case 8:
                        min_max.template operator()<int8_t>();
                        break;
                    case 16:
                        min_max.template operator()<int16_t>();
                        break;
                    case 32:
                        min_max.template operator()<int32_t>();
                        break;
                    case 64:
                        min_max.template operator()<int64_t>();
                        break;
                    }
                    break;
                case Numeric::N_Float:
            ...
            case m::Function::FN_SUM:
            {
                ...
                case Numeric::N_Decimal:
                    ...
                    default:
                        M_unreachable("invalid size");
                    case 8:
                        sum.template operator()<int8_t>();
                        break;
                    case 16:
                        sum.template operator()<int16_t>();
                        break;
                    case 32:
                        sum.template operator()<int32_t>();
                        break;
                    case 64:
                        sum.template operator()<int64_t>();
                        break;
                    }
                    break;
                case Numeric::N_Float:
        ...
        for (std::size_t idx = 0; idx < num_keys; ++idx)
        {
            visit(overloaded{
                      [&](const Boolean &)
                      { store.template operator()<bool>(idx); },
                      [&](const Numeric &n)
                      {
                          switch (n.kind)
                          {
                          ...
                          case Numeric::N_Decimal:
                              ...
                              default:
                                  M_unreachable("invalid size");
                              case 8:
                                  store.template operator()<int8_t>(idx);
                                  break;
                              case 16:
                                  store.template operator()<int16_t>(idx);
                                  break;
                              case 32:
                                  store.template operator()<int32_t>(idx);
                                  break;
                              case 64:
                                  store.template operator()<int64_t>(idx);
                                  break;
                              }
                              break;
                          case Numeric::N_Float:
    ...
    M.child->execute(
        ...
        /* pipeline= */ [&]()
        {
                               ...
                               [&](NChar value) -> void
                               {
                                   ...
                                   auto addr_differs = strncmp(
                                       /* left=  */ NChar(addr, value.can_be_null(), value.length(),
                                                          value.guarantees_terminating_nul()),
                                       /* right= */ NChar(key_addr, value.can_be_null(), value.length(), value.guarantees_terminating_nul()),
            ...
            IF(pred ? Select(*pred, cond, false) : cond)
            { // ignore entries for which predication predicate is not fulfilled
    ...
    BLOCK_OPEN(fn.body())
    {
        ...
        for (auto &e : M.grouping.schema().deduplicate())
        {
            ...
            { // part of key or already computed aggregate
                std::visit(overloaded{
                               [&]<typename T>(Expr<T> value) -> void
                               {
                                   if (value.can_be_null())
                               ...
                               { M_unreachable("SIMDfication currently not supported"); },
                               [](std::monostate) -> void
                               { M_unreachable("invalid reference"); },
                           },
...
void Aggregation::execute(const Match<Aggregation> &M, setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    ...
    if (std::any_of(avg_aggregates.begin(), avg_aggregates.end(), [](auto &i)
                    { return i.second.compute_running_avg; }))
    ...
    for (auto &e : M.aggregation.schema().deduplicate())
    {
        ...
        { // already computed aggregate
            std::visit(overloaded{
                           ...
                           [](auto) -> void
                           { M_unreachable("only scalar and non-string values must occur"); },
                           [](std::monostate) -> void
                           { M_unreachable("invalid reference"); },
                       },
                       results.get(e.id)); // do not extract to be able to access for not-yet-computed AVG aggregates
...
void NestedLoopsJoin<Predicated>::execute(const Match<NestedLoopsJoin> &M, setup_t setup, pipeline_t pipeline,
                                          teardown_t teardown)
{
    std::cout << "NestedLoopsJoin: EXECUTED" << std::endl;
    double cardinality_count = 0;
    ...
    std::cout << "num_left_children (WasmOperator.cpp:5064) : " << num_left_children << std::endl;
        ...
        FUNCTION(nested_loop_join_child_pipeline, void(void))
        {
                ...
                buffers.emplace_back(
                    ...
                    /* pipeline=      */ [&, pipeline = std::move(pipeline)]()
                    {
                        if constexpr (Predicated) {
                            ...
                            cardinality_count++;
                            std::cout << "NestedLoopsJoin predicated: Emitting tuple #" << cardinality_count << std::endl;
                            pipeline();
                        } else {
                            M_insist(CodeGenContext::Get().num_simd_lanes() == 1, "invalid number of SIMD lanes");
                            IF (CodeGenContext::Get().env().compile<_Boolx1>(M.join.predicate()).is_true_and_not_null()) {
                                cardinality_count++;
                                std::cout << "NestedLoopsJoin: Emitting tuple #" << cardinality_count << std::endl;
                                pipeline();
                            };
                        } },
                    /* teardown=      */ teardown_t::Make_Without_Parent());
            ...
            else
            {
                ...
                buffers.emplace_back(
                    ...
                    /* setup=         */ setup_t::Make_Without_Parent(),
                    /* pipeline=      */ [&]()
                    { buffers.back().resume_pipeline_inline(); },
                    /* teardown=      */ teardown_t::Make_Without_Parent());
            }
            ...
            M.children[i]->execute(
                /* setup=    */ setup_t::Make_Without_Parent([&]()
                                                             { buffers.back().setup(); }),
                /* pipeline= */ [&]()
                { buffers.back().consume(); },
                /* teardown= */ teardown_t::Make_Without_Parent([&]()
                                                                { buffers.back().teardown(); }));
        }
...
{
    std::cout << "SimpleHashJoin: EXECUTED" << std::endl;
    // TODO: determine setup
    ...
    M.children[1]->execute(
        ...
        /* teardown= */ teardown_t(std::move(teardown), [&]()
                                   { ht->teardown(); }));
}
    ...
    teardown_t teardown)
{
    ...
    std::cout << "SortMergeJoin: EXECUTED" << std::endl;
        ...
        FUNCTION(sort_merge_join_parent_pipeline, void(void)) // create function for parent pipeline
        {
            ...
            M.children[0]->execute(
                /* setup=    */ setup_t::Make_Without_Parent([&]()
                                                             { buffer_parent->setup(); }),
                /* pipeline= */ [&]()
                { buffer_parent->consume(); },
                /* teardown= */ teardown_t::Make_Without_Parent([&]()
                                                                { buffer_parent->teardown(); }));
        }
        ...
        FUNCTION(sort_merge_join_child_pipeline, void(void)) // create function for child pipeline
        {
            ...
            M.children[1]->execute(
                /* setup=    */ setup_t::Make_Without_Parent([&]()
                                                             { buffer_child->setup(); }),
                /* pipeline= */ [&]()
                { buffer_child->consume(); },
                /* teardown= */ teardown_t::Make_Without_Parent([&]()
                                                                { buffer_child->teardown(); }));
        }
    ...
    for (auto &clause : M.join.predicate())
    {
        ...
        M_insist(is<const Designator>(binary.lhs), "invalid equi-predicate");
        M_insist(is<const Designator>(binary.rhs), "invalid equi-predicate");
        auto [expr_parent, expr_child] = M.parent.schema().has(Schema::Identifier(*binary.lhs)) ? std::make_pair(binary.lhs.get(), binary.rhs.get()) : std::make_pair(binary.rhs.get(), binary.lhs.get());
    ...
    WHILE(tuple_id_parent < size_parent and tuple_id_child < size_child)
    { // neither end reached
                                                                                          ...
                                                                                          unsigned level) const
{
    ...
    case 0:
        ...
        break;
    case 1:
        out << "sorting right input " << (CmpPredicated ? "predicated " : "");
        break;
    case 2:
        out << "sorting left input " << (CmpPredicated ? "predicated " : "");
        break;
    case 3:
        out << "sorting both inputs " << (CmpPredicated ? "predicated " : "");
        break;
    }
...
#define DECLARE(CLASS)                        \
    ...
    {                                         \
        if constexpr (PreOrder)               \
            ...
            catch (visit_skip_subtree)        \
            {                                 \
                return;                       \
            }                                 \
        super::operator()(M);                 \
        if constexpr (not PreOrder)           \
            callback_(M);                     \
    }
